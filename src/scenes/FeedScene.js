...
export class FeedScene extends Phaser.Scene {
  ...
  create() {
    ...
    // 요청 만들기 (레벨 1:1~3+점 / 2:1~5+점 / 3:1~5 / 4:1~8 / 5:합성)
    this.request = [];
    if (lvl >= 5 && Math.random() < 0.6) {
  ...
  layout() {
    ...
    if (this.request.length > 1) {
      ...
      const t = this.add.text(this.bubbleImg.x + 4, this.bubbleImg.y - 12, `× ${r0.count}`, textStyle(fs * 0.85)).setOrigin(0, 0.5).setDepth(9);
      this.bubbleIcons.push(icon, t);
    }
    // 수량 점 병기 (저레벨)
    this.dotG.clear();
    ...
    this.time.delayedCall(danceDelay, () => {
      ...
      this.time.delayedCall(1100, () => {
        ...
        ui.celebrate({
          ...
          msg: growMsg || (this.eaterChar ? `${this.eaterChar.n}가 배불러서 춤을 춰요!` : '별님이가 배불러요!'),
          speakMsg: growMsg,
